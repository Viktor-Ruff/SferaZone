...
class PeopleAdapter constructor(private val listPeople: List<PeopleModel>) :
    ListAdapter<PeopleModel, PeopleAdapter.PeopleViewHolder>(DiffUtilCallback()) {


    private var onItemOnClickListener: ((PeopleModel) -> Unit)? = null

    fun setOnItemOnClickListener(people: (PeopleModel) -> Unit) {
        onItemOnClickListener = people
    }
    ...
    class DiffUtilCallback : DiffUtil.ItemCallback<PeopleModel>() {
